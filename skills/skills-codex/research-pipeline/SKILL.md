...
- New run: `python3 "$RUN_STATE" start . "$RUN_ID" --executor codex-gpt-5.5 --provisional-advances --phases "idea-discovery,experiment-bridge,auto-review-loop,summary,paper-writing"` (the `--provisional-advances` policy is what lets a same-family provisional verdict close a phase for resume — without it, mainline semantics apply and provisional phases stay open).
- Resume: `python3 "$RUN_STATE" resume . "$RUN_ID"`; restart the returned phase.
- Each phase: mark `running`, then `done --artifact <path>`.
- A fresh Codex reviewer PASS uses `mark-provisional --reviewer gpt-5.5
...
3. **Fresh-agent code review** — gpt-5.5 xhigh reviews the implementation in a new context; base result is same-family provisional
