...
- MAX_REVIEW_ROUNDS = 3
- REVIEWER_MODEL = `gpt-5.5` via Codex reviewer agent, reasoning effort
  `xhigh` for this deep-audit skill (capability fallback never below `xhigh`)
- **REVIEWER_BACKEND = `codex`** — Default: Codex reviewer agent (`spawn_agent`, xhigh — deep-audit tier). Override with `— reviewer: oracle-pro` for GPT-5.5 Pro via Oracle MCP. See `shared-references/reviewer-routing.md`.
...
- **Codex reasoning always xhigh** (deep-audit tier): never below `xhigh` — only the capability fallback in `reviewer-routing.md` may step down, and only on explicit capability errors.
